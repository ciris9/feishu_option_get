...
func GetOpPrivilegeSystemNameOptions(ctx iris.Context) {
	tableID := ctx.URLParam("table_id")
	appToken := ctx.URLParam("app_token")

	opPrivilegesService := service.NewOpPrivilegesService()
	...
	options, err := opPrivilegesService.GetOpPrivilegeSystemNameOptions(approveRequest)
	if err != nil {
